...
const tiers = [
  ...
  { name: "Destek sponsoru", tone: "support", logos: [{ name: "Hidromek", src: "/logos/hidromek.png", href: "https://www.hidromek.com/", wide: true }, { name: "Baş Yapı", src: "/logos/besyapi.jpg", href: "https://besyapi.com/", wide: false }] },
];
...
export default function TrialSponsors() {
  return <section id="sponsors" className={s.section}>
    ...
    <div className={s.sponsorWall}>{tiers.map((tier) => <div className={s.tier} key={tier.name} data-tone={tier.tone}>
      ...
      <div className={s.logos}>{tier.logos.map(logo => <a className={s.logo} key={logo.name} href={logo.href} target="_blank" rel="noopener noreferrer" aria-label={`${logo.name} resmi sitesi (yeni sekme)`}><Image src={logo.src} alt={logo.name} width={280} height={120} className={logo.wide ? s.wide : s.tall} /><span>{logo.name} <span aria-hidden="true">↗</span></span></a>)}</div>
    </div>)}</div>
